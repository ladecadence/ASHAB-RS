...
impl Config {
    ...
    pub fn open(&mut self) -> Result<(), Error> {
        ...
        let section_mission = match conf.section(Some("mission".to_owned())) {
            Some(s) => s,
            None => return Err(Error::new(ErrorKind::Other, "Section mission not found")),
        };
        ...
        self.separator = section_mission.get("separator").unwrap().to_string();
        self.packet_repeat = section_mission.get("packet_repeat").unwrap().parse::<u32>().unwrap();
        self.packet_delay = section_mission.get("packet_delay").unwrap().parse::<u32>().unwrap();
        ...
        let section_gps = match conf.section(Some("gps".to_owned())) {
            Some(s) => s,
            None => return Err(Error::new(ErrorKind::Other, "Section gps not found")),
        };

        self.gps_serial_port = section_gps.get("serial_port").unwrap().to_string();
        self.gps_speed = section_gps.get("speed").unwrap().parse::<u32>().unwrap();
        ...
        let section_temp = match conf.section(Some("temp".to_owned())) {
            Some(s) => s,
            None => return Err(Error::new(ErrorKind::Other, "Section temp not found")),
        };

        self.temp_internal_addr = section_temp.get("internal_addr").unwrap().to_string();
        self.temp_external_addr = section_temp.get("external_addr").unwrap().to_string();
        ...
        let section_path = match conf.section(Some("paths".to_owned())) {
            Some(s) => s,
            None => return Err(Error::new(ErrorKind::Other, "Section paths not found")),
        };

        self.path_main_dir = section_path.get("main_dir").unwrap().to_string();
        self.path_images_dir = section_path.get("images_dir").unwrap().to_string();
        self.path_log = section_path.get("log").unwrap().to_string();

        // get ssdv section
        let section_ssdv = match conf.section(Some("ssdv".to_owned())) {
            Some(s) => s,
            None => return Err(Error::new(ErrorKind::Other, "Section ssdv not found")),
        };
